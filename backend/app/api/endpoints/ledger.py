...
def create_log(log: schemas.OperationalLogCreate, response: Response, db: Session = Depends(get_db), current_user: models.User = Depends(require(Permission.LOG_CREATE))):
...
def read_logs(skip: int = 0, limit: int = 100, db: Session = Depends(get_db), current_user: models.User = Depends(require(Permission.LOG_READ))):
...
def read_transactions(skip: int = 0, limit: int = 100, db: Session = Depends(get_db), current_user: models.User = Depends(require(Permission.FINANCE_READ))):
    return ledger_service.get_financial_transactions(db=db, farm_id=current_user.farm_id, skip=skip, limit=limit)

@router.get("/summary")
def get_summary(db: Session = Depends(get_db), current_user: models.User = Depends(require(Permission.FINANCE_READ))):
...
def reverse_log(log_id: int, db: Session = Depends(get_db), current_user: models.User = Depends(require(Permission.LOG_REVERSE))):
