...
def create_link(db: Session, farm_id: int, label: str | None) -> tuple[models.ShareToken, str]:
...
def get_report_by_token(db: Session, token: str) -> dict:
    ...
    report can only ever be the token's own farm. Unknown or revoked tokens
    resolve to nothing (404), which is also how revocation takes effect.
    """
    link = (
        db.query(models.ShareToken)
        .filter(
    ...
    if link is None:
        raise NotFoundError("Share link is invalid or has been revoked")
